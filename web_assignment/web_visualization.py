...
def get_data(county: str) -> pd.DataFrame:
    ...
    """
    file = os.path.join("data_files", f"antall-meldte-covid-19-{county.lower()}.csv")
...
def plot_reported_cases(county: str, start: Optional[str] = None, end: Optional[str] = None) -> alt.Chart:
    ...
    title = 'Oversikt over komulativt antall smitttede i ' + county + ' fra ' + str(start) + ' til ' + str(end)
...
def plot_both(county: str, start: Optional[str] = None, end: Optional[str] = None) -> alt.Chart:
    ...
    return alt.layer(reported_bar, cumulative_line).resolve_scale(
        y='independent'
    )
